...
Class Ticket_status extends MY_Controller {
    ...
    public function show_all_ticket_status() {
        $show_result = $this->get_all_ticket_status();
        $data['show_result'] = $show_result;

        $this->load->view('show_ticket_status.html', $data);
    }

    public function get_ticket_status($status_id) {
        if ($status_id != 0) {
